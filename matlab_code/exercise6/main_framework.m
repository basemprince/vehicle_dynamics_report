...
const_speed = true;
freq = 0.001; % 1/s
req_speeds = [20,30,40,50,60,70,80];
req_speeds_k = req_speeds;
req_speeds = req_speeds/3.6;
look_ahead_list = [5,10,15,20,25,30];
if const_speed
...
for ind=1:length(req_speeds)
    ...
    for ind2 = 1: length(look_ahead_list)
        ...
        if (speed_req_k ==70 &&  look_ahead == 15) || (speed_req_k ==80 &&  look_ahead < 20)
            int = 1
        else
            total = total +1;
            clothoidBasedParams = clothoidBasedControllerParams(kus,look_ahead);
            fprintf('Starting Simulation # %d [%d km/h] w/ look_ahead %d kus %f\n', total, round(speed_req*3.6),look_ahead,kus);
            tic;
            % Simulink simulation
...
%         fprintf("speed: %d lookup: %d", error_data{i}.speed, error_data{i}.look_ahead);
    ...
    writetable(T,'error_speed_lookahead_table.csv');
    
    %% bar graph for tracking error [clothoid]
    
    pivot_table = zeros(length(req_speeds_k),length(look_ahead_list)+1);
    for k =1:length(req_speeds_k)
        pivot_table(k,1) = req_speeds_k(k);
        for j =1:length(look_ahead_list)
            for i = 1:length(error_data)
                if error_data{i}.reached_flag
                    if error_data{i}.speed == req_speeds_k(k)
                        if error_data{i}.look_ahead == look_ahead_list(j)
    ...
    xlim([15 75])
    set(gca,'fontsize',26)
    hleg = legend(string(look_ahead_list),'location','northwest');
    htitle = get(hleg,'Title');
    set(htitle,'String','look ahead')
    pbaspect([1 1 1])
