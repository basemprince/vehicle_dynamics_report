function postProcessing(model_sim,vehicle_data,Ts,road_data_sampled)
    ...
    plot(time_sim,squeeze(model_sim.req_speed.data)*3.6,'--','LineWidth',2)
    grid on
    title('$u$ [km/h]')
    legend('real','target')
    xlim([0 time_sim(end)])
    ...
    ax(4) = subplot(224);
    hold on
    plot(time_sim,rad2deg(delta_D)/tau_D,'LineWidth',2)
    plot(time_sim,delta_fr,'LineWidth',2)
    plot(time_sim,delta_fl,'LineWidth',2)
    grid on
    ...
    plot(road_data_sampled(:,1),road_data_sampled(:,2),'-b')
    ...
    grid on
    legend('target path','vehicle path')
