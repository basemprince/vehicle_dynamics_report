...
% Set to 0 to disable route planning and load a precomputed route
enable_routePlan = 0;
% set to 0 to disable simulink
enable_simulation = 1;
...
% frequency of online plotting 
plot_freq = 1;
% Set to 1 in order to enable a zoomed view in online simulation plots
enable_zoom = 0;

% allow change of interlpolation sampling on precomputed route
change_samp = true;
...
% loading handling behaviour, and optimized lookup values for clothoid and
% pure pursuit from exercise 6
...
% --------------------------------------
%% variables to loop through for testing
% --------------------------------------

% for route planning optimization [all tested]
...
% changing initial speed in initial conditions
X0(4) = init_speed;
speed_req = speeds_req(1);
% initializing clothoid based controller based on speed
...
% initializing stanley params
stanleyParams       = stanleyControllerParams(m_steers(1),0.1);

% initializing pure pursuit controller based on speed
...
if (enable_routePlan)
    ...
    route_results = {};
    % loop through all test variables [c_dist, iters, m_steer,
    % interp_samples]
    total = 0;
    ...
    % data re-formating for graph construction
...
if(enable_simulation)
    % Simulink simulation
    if(change_samp)
        % change the interpolation sampling for the pre-computed route
    ...
    for ind=1:length(speeds_req)
        ...
        % changing the variables to match test requirements
