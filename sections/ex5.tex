...
The slope of the line represents the under-steering gradient $\kus$, which describes the evolution of $\delta$ as $\ay$ increases. $\kus$ is positive if the vehicle is under-steering and negative when over-steering.

Increasing the speeds to 80 and 100 km/h resulted in an increase in the lateral acceleration $\ay$ as shown in Figure \ref{51c}. The vehicle nonetheless showed over-steering behaviour across all the simulated speeds. A comparison between the yaw-rate $\Omega$ and the desired steering angle $\deltd$ shows that $\Omega$ keeps increasing with higher speeds even though $\deltd$ is staying the same.
...
The results of all the coefficients for the fittings for the three runs are shown above. They are certainly not constant across the runs because they fit different curves. However, some are quite close, especially runs \# 2 and 3. This could be attributed to the similarity in the linear sections of the three curves as shown in Figure \ref{52d}.
        ...
        \begin{figure}[ht]
        \includegraphics[width=0.5\linewidth]{ex5/q2/ex-52d.eps}
        \centering
...
\end{equation}
        ...
        \begin{figure}[ht]
        \includegraphics[width=0.5\linewidth]{ex5/q3/ex-53e1.eps}
        \centering
...
In conclusion, the steering gradient $\kus$ is a function of multiple variables including ($\ay, u, \ax$). To predict handling behaviour more accurately, the vehicle was tested from 20 to 100 km/h using the sine steer maneuver with a $\deltd$ = $10^\circ$. The results are in Table \ref{tab:6.1} and Figure \ref{53d1}.

\begin{table}[ht]
    \centering
    \begin{tabular}{c | c}
      \textbf{$u$ [km/h]}  & \textbf{$\kus$} \\ \hline
      20 &  \num{-9.96E-05}    \\\hline
      30 &  \num{-0.000296518} \\\hline
      40 &  \num{-0.00059611}  \\\hline
      50 &  \num{-0.001027394} \\\hline
      60 &  \num{-0.001629399} \\\hline
      70 &  \num{-0.002448684} \\\hline
      80 &  \num{-0.003542939} \\\hline
      90 &  \num{-0.00498508}  \\\hline
      100 &  \num{-0.006867669}\\
    \end{tabular}
    \caption{Vehicle's $\kus$ for tested speeds with sine steer [$\deltd$ = $10^\circ$]}
    \label{tab:5.1}
\end{table}

        \begin{figure}[ht!]
        \includegraphics[width=0.47\linewidth]{ex5/q3/ex-53d1.eps}
        \centering
        \caption{$\kus$ results based on $u$ with sine steer [$\deltd$ = $10^\circ$]}
        \label{53d1}
        \end{figure}
...
\begin{table}[ht]
    ...
    \begin{tabular}{p{0.5\linewidth} | p{0.5\linewidth}}
      \textbf{Sine steer}  & \textbf{Constant Steer} \\ \hline
      More difficult to maintain steady state as the steering angle is changing constantly &
      Easier to maintain steady state considering that $u$ is easier to regulate \\ \hline
      
      Needs a robot to perform &
      Easily performed by a human driver \\ \hline
      
      Perform left and right turns on the same run &
      Only perform either left or right turns per run \\ \hline

      Requires a larger area to perform \newline [going straight and steering left to right] &
      A smaller area is required to perform \newline
      [going in circles] 
    \end{tabular}
    \caption{Pros And Cons of the constant steer and sine steer tests}
    \label{tab:5.3}
\end{table}