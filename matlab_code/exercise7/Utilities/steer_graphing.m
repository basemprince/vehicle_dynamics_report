...
S.m_steer_test_4 = load('results/m_steer_test_4eg.mat');
S.m_steer_test_5 = load('results/m_steer_test_5eg.mat');
S.m_steer_test_6 = load('results/m_steer_test_6eg.mat');
S.m_steer_test_7 = load('results/m_steer_test_7eg.mat');
S.m_steer_test_8 = load('results/m_steer_test_8eg.mat');
...
for i = 4:2:8
    ...
    hold on
    %plot(planner)
    plot(S.(fn).(fn){1}{1}.costmap)
    plot(S.(fn).(fn){1}{1}.refPath,'DisplayName','Planned Path')
...
for i = 4:2:8
    ...
    time_sim = S.(fn).(fn){2}{1}.states.u.time;
    x_CoM      = S.(fn).(fn){2}{1}.states.x.data;
    y_CoM      = S.(fn).(fn){2}{1}.states.y.data;
    psi        = S.(fn).(fn){2}{1}.states.psi.data;
    u        = S.(fn).(fn){2}{1}.states.u.data;
    v        = S.(fn).(fn){2}{1}.states.v.data;

    N = length(time_sim);
    vehRoute = ClothoidList();
    numOfClothoids_route = size(S.(fn).(fn){1}{1}.refRoute_fewPoints,1);
    for j = 1:numOfClothoids_route-1
        vehRoute.push_back_G1(S.(fn).(fn){1}{1}.refRoute_fewPoints(j,1),...
            S.(fn).(fn){1}{1}.refRoute_fewPoints(j,2),...
            S.(fn).(fn){1}{1}.refRoute_fewPoints(j,3),...
            S.(fn).(fn){1}{1}.refRoute_fewPoints(j+1,1),...
            S.(fn).(fn){1}{1}.refRoute_fewPoints(j+1,2),...
            S.(fn).(fn){1}{1}.refRoute_fewPoints(j+1,3)); 
    end
    ...
    plot(S.(fn).(fn){1}{1}.refPath_poses_fewPoints(:,1),S.(fn).(fn){1}{1}.refPath_poses_fewPoints(:,2),'go','MarkerFaceColor','g','MarkerSize',8) %,'DisplayName','Route'

    % Plot original (not interpolated) reference route 
    plot(S.(fn).(fn){1}{1}.refRoute_points_orig(:,1),S.(fn).(fn){1}{1}.refRoute_points_orig(:,2),'o','Color',color('orange'),'MarkerFaceColor',color('orange'),'MarkerSize',8) %,'DisplayName','Interpolated Route'
