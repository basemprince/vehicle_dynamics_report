...
The steering angles for the same chosen runs are plotted in Figure \ref{64ff}. The steering angle for pure pursuit exhibited major spikes towards unrealistic numbers, which could be due to the lack of a maximum steer angle. However, it also indicates that the controller needs dampening to counter-react the sudden change in the steering angle as it is trying to instantly follow the reference trajectory. It would not be a pleasant ride using the pure pursuit.   
        
The clothoid based controller showed better transition between angles, which could make it a more comfortable ride compared to the pure pursuit.

Both of the Stanley controllers showed the same pattern. Some oscillation were present at the higher speed run.

Overall, from a tracking error prospective, the pure pursuit lateral controller is a clear winner. From a steering behaviour prospective, the clothoid based controller is better than the rest.
...
\begin{table}[ht!]
    ...
    \setlist{leftmargin=*}
    \begin{itemize}[noitemsep]
        \item steering behaviour is erratic
